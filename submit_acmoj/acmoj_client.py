...
def main():
    ...
    submit_parser.add_argument("--problem-id", type=str, required=True, help="Problem ID (can be comma-separated)")
    ...
    if args.command == "submit":
        ...
        problem_ids = args.problem_id.split(',')
        results = []
        for pid in problem_ids:
            try:
                pid_int = int(pid.strip())
                res = client.submit_code(pid_int, args.language, code_text)
                results.append(res)
            except ValueError:
                print(f"Error: Invalid problem ID '{pid}'")
                exit(1)
